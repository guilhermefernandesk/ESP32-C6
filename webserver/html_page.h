...
<html>
  ...
  <body>
    ...
    <script>
      ...
      async function toggleLed(element) {
        element.checked ? await ligarLED() : await desligarLED();
      }
      async function ligarLED() {
        try {
          ...
          const data = await response.text();
          if (!response.ok) {
            throw new Error("Erro ao ligar o LED");
          }
          atualizarEstado(data);
        } catch (error) {
          console.error(error);
      ...
      async function desligarLED() {
        try {
          ...
          const data = await response.text();
          if (!response.ok) {
            throw new Error("Erro ao desligar o LED");
          }
          atualizarEstado(data);
        } catch (error) {
          console.error(error);
